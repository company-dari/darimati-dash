...
<a href="inventory/">📦 창고 작업 가이드<span>양품화 입력·출고 표시·상황별 기록 위치 · 전체 문서는 안쪽에 · 바로 사용</span></a>
<a href="utm/">🔗 UTM·광고명 생성기<span>바로 사용</span></a>
<a href="qr/">🔲 QR 관리<span>인쇄물용 QR 생성·목적지 교체·스캔 집계 · 암호 필요</span></a>
</div></body></html>
"""

def build_qr():
    """QR 링크표를 q/links.js 로 구워 둔다 — 구글이 죽어도 인쇄물이 살도록."""
    sys.path.insert(0, os.path.join(HERE, "qr"))
    try:
        import snapshot
